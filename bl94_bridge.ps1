...
function ConvertFrom-BridgeTimestamp {
    param([string]$Value)
...
function Get-BridgePayloadFromRequest {
    ...
    if ($contentType -match '(?i)application/json') {
        try {
            ...
            $payload.SentAtUtc = ConvertFrom-BridgeTimestamp ([string]$json.sentAtUtc)
            $payload.FirstSeenAtUtc = ConvertFrom-BridgeTimestamp ([string]$json.firstSeenAtUtc)
